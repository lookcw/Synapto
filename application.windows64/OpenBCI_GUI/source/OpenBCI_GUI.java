...
class W_fft extends Widget {
  ...
  int FFT_indexLim = PApplet.parseInt(1.0f*xMax*(Nfft/getSampleRateSafe()));   // maxim value of FFT index
  ...
  public void update(){
    ...
    for (int i = 0; i < fft_points.length; i++) {
      for (int j = 0; j < FFT_indexLim + 2; j++) {  //loop through frequency domain data, and store into points array
        ...
        powerAtBin = new GPoint((1.0f*getSampleRateSafe()/Nfft)*j, fftBuff[i].getBand(j));
...
class W_Focus extends Widget {
  ...
  int cBack = 0xffffffff;   //white
  int cDark = 0xff032e61;   //medium/dark blue
  int cMark = 0xff306aaf;    //lighter blue
  int cFocus = 0xff020916;   //peach
  int cWave = 0xffffdd3a;    //yellow
