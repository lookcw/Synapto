...
delta = zeros(length(data(:,1)),7);
theta = zeros(length(data(:,1)),7);
alpha = zeros(length(data(:,1)),7);
beta  = zeros(length(data(:,1)),7);
gamma = zeros(length(data(:,1)),7);
...
Fstop2 = 55.4;            % Second Stopband Frequency
Dstop1 = 0.01;            % First Stopband Attenuation
Dpass  = 0.057501127785;  % Passband Ripple
Dstop2 = 0.01;            % Second Stopband Attenuation
[N, Fo, Ao, W] = firpmord([Fstop1 Fpass1 Fpass2 Fstop2]/(Fs/2), [0 1 0], [Dstop1 Dpass Dstop2]);
