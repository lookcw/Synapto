...
def regionalize_average_features(features_file):
    ...
    if True or not os.path.exists(feature_avg_filename):
        dataset = pd.read_csv(features_file)
        # dataset = pd.read_csv(features_file).drop(columns=['patient num'])
        data = dataset[['instance code', 'patient num', 'class']].copy()
        dataset = dataset.drop(
            columns=['instance code', 'patient num', 'class', 'instance num'], errors='ignore')
        assert len(dataset.columns) == 21
        ...
        for region_name in regions:
            data[region_name] = dataset.iloc[:,
                                             regions[region_name]].mean(axis=1)
        adhc = data.pop('class')
        data['class'] = adhc
