...
def get_labels_from_folder(CONFIG):
    ...
    eeg_data_filename = next(filename for filename in os.listdir(CONFIG['positive_folder_path']) if filename.endswith('.csv'))
    print(os.path.join(CONFIG['positive_folder_path'], eeg_data_filename))
    whole_data_set = genfromtxt(os.path.join(CONFIG['positive_folder_path'], eeg_data_filename), delimiter=',')
