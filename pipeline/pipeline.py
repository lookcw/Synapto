...
if not startAtFS:
	...
	if (RECURR):
		features_path = sys.path[0] + '/FeatureSets/' + recurrParamToFilename(data_type, num_instances ,num_timePoints, epochs_per_instance)
	else:
		#unique identifier for different input parameters
		features_path = sys.path[0] + '/FeatureSets/'+  paramToFilename(data_type, num_instances ,num_timePoints, epochs_per_instance)

	#define features and reduced_features paths
	
	reduced_features_path = sys.path[0] + '/ReducedFeatureSets/'+ paramToFilename(data_type, num_instances ,num_timePoints, epochs_per_instance)
