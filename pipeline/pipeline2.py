...
DATA_TYPE_TO_FOLDERS = {
    ...
    'NCRegions': ('New_Castle_Data/HC_regions', 'New_Castle_Data/AD_regions'),
...
for i in range(1, len(sys.argv), 2):
    if str(sys.argv[i]) == "-h":
        ...
        config['feature_filename'] = sys.argv[i+1].split('/')[-1]
        feature_paths = [os.path.join(FEATURE_SET_FOLDER,config['feature_filename'])]
        feature_filenames = [config['feature_filename']]
...
results = [get_results(model, feature_set, config, config_feature, feature_filename) for (
