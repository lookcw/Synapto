...
def average_heatmap(correlation_matrix, regionalization, regionalization_function):
    ...
    for i in range(len(sorted_region_names)):
        for j in range(len(sorted_region_names)):
            if i >= j:
                avg = _add_correlation(corre_regions[sorted_region_names[i]],
                                       corre_regions[sorted_region_names[j]],
                                       correlation_matrix,
                                       regionalization_function)
                regionalized_matrix[i][j] = avg
                regionalized_matrix[j][i] = avg
    return regionalized_matrix[np.triu_indices(len(corre_regions), 0)]


def _add_correlation(region1, region2, correlation_matrix, regionalization_function):
    region1, region2 = np.array(region1), np.array(region2)
    return regionalization_function(correlation_matrix[region1[:, None], region2])
